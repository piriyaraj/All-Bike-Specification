...
def findNoOfPost(url):
    ...
    for i in trs[1:]:
        if(i.findAll("td")[1].text == ""):
            continue
        atag = i.findAll("a")[0]
        modelName = atag.get_attribute_list("href")[0].split("models/")[1].split(".php")[0]
...
def run():
    ...
    for modelLink in modelsLinks[:5]:
        ...
        # except:pass
        print("\n\n>>> "+str(modelsLinks.index(modelLink)+1) + "/"+str(len(modelsLinks))+" "+modelforfile, end="")

        if(noOfPostInModelFromFirebase[modelforfile] == noOfPostInModelFromsite[modelforfile]):
            print(" ||| NO New post found")
            continue

        print(" ||| NO of Post: ",noOfPostInModelFromsite[modelforfile]-noOfPostInModelFromFirebase[modelforfile])
        ...
        insertData('data/crewelData',{modelforfile: len(postlinks)}, dataBase, format='patch')
        for postlink in postlinks:
            ...
            if(pos == -1):
                ...
                insertData('data/models/'+modelforfile,{"url": postlink}, dataBase)

                print("======>>>"+postlink)

        # fileModlelinks.write(modelLink+"\n")
        insertData('data/modelsLinks',{modelforfile: modelLink}, dataBase, format='patch')
